...
(function($) {
    ...
    var NS = "cmp", TEMPLATE = 'item-template', ERROR_CLASS = 'cmp-portallister__item--error', ATTR_PREFIX = "data-"
            + NS + "-hook-" + TEMPLATE, ATTR_PREFIX_MENU = "data-" + NS + "-hook-menu-template";
    ...
    var addTextWithTooltip = function(element, text, tooltip) {
        ...
        var operation_dataStr = event.currentTarget.getAttribute('operation_data'), operation_data = JSON
                .parse(operation_dataStr);
    ...
    var Menu = {
        ...
        addMenuItem : function(operation, menuItemTemplate, listEl) {
            ...
            var item = el.querySelector(Menu.selectors.item), itemLabel = item.querySelector(Menu.selectors.itemLabel);
        ...
        constructMenu : function() {
            ...
            var menuEl = el.querySelector(Menu.selectors.menuContainer), menuItemTemplate = el
                    .querySelector(Menu.selectors.menuItemTemplate).innerHTML, listEl = el
                    .querySelector(Menu.selectors.list), operationsStr = Menu.containerElement
                    .getAttribute("operations"), operationModelId = Menu.containerElement
                    .getAttribute("operation_model_id"), queryPath = Menu.queryPath, operations = JSON
                    .parse(operationsStr), addListItem = function(operation) {
    ...
    var ItemAPI = {
        createAndInject : function(componentConfig, data) {
            var template = componentConfig.itemTemplate, container = componentConfig.resultsNode, menu = componentConfig.menu;
            /* data has schema:
                {
                    title,
                    description,
            ...
            var item = el.querySelector(selectors.self), titleElem = el.querySelector(selectors.title), linkElem = el
                    .querySelector(selectors.formLink), descElem = el.querySelector(selectors.description), thumbnailElem = el
                    .querySelector(selectors.thumbnail), operationsElem = el.querySelector(selectors.operations), timeInfoElem = el
                    .querySelector(selectors.timeinfo), itemContainer = container.querySelector(selectors.container);
        ...
        init : function(config) {
            var itemTemplate = config.itemTemplate, container = config.resultsNode;
            ...
            var resultContainer = el.querySelector(selectors.container), includeElements = el
                    .querySelector(selectors.includes);
        ...
        clear : function(template, container) {
            ...
            var originalState = el.querySelector(selectors.container), resultContainer = container
