...
* Supports the following recaptcha versions:
  ...
  * v3
    * Only invisible behaviour is supported (badge-only, no visible challenge); the token is
      auto-fetched at submit. Requires the FT_FORMS-27648 feature toggle to be enabled.
  * Enterprise
...
10. `./recaptchaSize` - defines the size attribute of Google reCAPTCHA. This property is only valid for reCAPTCHA v2 and the checkbox keys of reCAPTCHA enterprise. This property will be disabled for reCAPTCHA Enterprise score-based keys and for reCAPTCHA v3 (both invisible).
