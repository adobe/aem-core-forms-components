...
(function($, Granite) {
    ...
    function handleDownloadDor(dialog) {
        ...
        if (downloadButton) {
            var performDownload = function(event) {
                if (event) {
    ...
    function handleDialogSubmit(dialog) {
        ...
        if (form.length && performDownload) {
            form.on("submit", function(event) {
                var dorTypeField = dialog.find(DOR_TYPE_FIELD)[0];
                var dorTemplateRefField = dialog.find('input[name="./dorTemplateRef"]')[0];
                var hasExistingTemplate = dorTemplateRefField && dorTemplateRefField.value;
                ...
                if (dorTypeField && dorTypeField.value === DOR_GENERATE && !getDownloadClicked() && (!hasExistingTemplate || autoDorFieldsChanged)) {
                    event.preventDefault();
                    performDownload();
