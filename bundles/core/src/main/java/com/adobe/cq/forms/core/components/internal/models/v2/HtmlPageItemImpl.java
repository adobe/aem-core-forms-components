...
public class HtmlPageItemImpl implements HtmlPageItem {
    ...
    public HtmlPageItem.Element getElement() {
        if (element == null) {
            if (config != null) {
                element = HtmlPageItem.Element.fromString(config.element());
            } else {
                // Support the former node structure: see com.adobe.cq.wcm.core.components.config.HtmlPageItemsConfig
                element = HtmlPageItem.Element.fromString(properties.get(HtmlPageItem.PN_ELEMENT, String.class));
            }
        }
        return element;
    }
    ...
    public HtmlPageItem.Location getLocation() {
        if (location == null) {
            if (config != null) {
                location = HtmlPageItem.Location.fromString(config.location());
            } else {
                // Support the former node structure: see com.adobe.cq.wcm.core.components.config.HtmlPageItemsConfig
                location = HtmlPageItem.Location.fromString(properties.get(HtmlPageItem.PN_LOCATION, String.class));
            }
        }
        return location;
    }
    ...
    private void addAttributes(String name, String value) {
        if ((getElement() == HtmlPageItem.Element.LINK && HtmlPageItem.PN_HREF.equals(name)) ||
            (getElement() == HtmlPageItem.Element.SCRIPT && HtmlPageItem.PN_SRC.equals(name))) {
