...
/**
 * Utility methods for resolving Next Gen Dynamic Media (NGDM) asset references (of the form
 * {@code /urn:aaid:aem:<asset-id>/<seo-name>.<format>}) into delivery URLs.
 */
public final class NgdmImageUtils {
    ...
    public static boolean isNgdmImageReference(String fileReference) {
        ...
        return StringUtils.isNotEmpty(StringUtils.substringBefore(withoutLeadingSlash, "/"))
            && StringUtils.isNotEmpty(StringUtils.substringAfter(withoutLeadingSlash, "/"));
    }
    ...
    public static String buildNgdmImageSrc(String fileReference, NextGenDynamicMediaConfig nextGenDynamicMediaConfig) {
        String withoutLeadingSlash = fileReference.substring(1);
        String assetId = StringUtils.substringBefore(withoutLeadingSlash, "/");
        String assetFileName = StringUtils.substringAfter(withoutLeadingSlash, "/");
        ...
        boolean hasExtension = StringUtils.contains(assetFileName, ".");
        String assetName = hasExtension ? StringUtils.substringBeforeLast(assetFileName, ".") : assetFileName;
        String assetExtension = hasExtension ? StringUtils.substringAfterLast(assetFileName, ".")
            : DEFAULT_NGDM_ASSET_EXTENSION;

        String imageDeliveryPath = nextGenDynamicMediaConfig.getImageDeliveryBasePath()
            .replace(PATH_PLACEHOLDER_ASSET_ID, assetId).replace(PATH_PLACEHOLDER_SEO_NAME, assetName)
            .replace(PATH_PLACEHOLDER_FORMAT, assetExtension);

        return "https://" + nextGenDynamicMediaConfig.getRepositoryId() + imageDeliveryPath + "?width="
            + DEFAULT_NGDM_ASSET_WIDTH + "&preferwebp=true";
    }
}
