...
public class TextImplTest {
    ...
    void testRichTextAbsentFromJcr() throws Exception {
        // PATH_TEXT has no textIsRich in JCR — isRichText() must return false (interface default),
        // and "richText" must be absent from JSON output (not serialized when null).
        ...
        assertFalse("richText must not appear in JSON when textIsRich is absent from JCR", json.contains("\"richText\""));
    }
