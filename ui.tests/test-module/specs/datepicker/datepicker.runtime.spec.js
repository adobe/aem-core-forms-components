...
describe("Form Runtime with Date Picker", () => {
    ...
    it("Test on datePicker with editFormat 'd/M/Y' and displayFormat 'EEEE, d MMMM, y'", () => {
        ...
        // check if the same date is persisited by openening calender
        cy.get(`#${datePicker7}`).find(".datepicker-calendar-icon").should("be.visible").click().then(() => {
            cy.get("#li-day-11").should("have.class", "dp-selected"); 
        });
