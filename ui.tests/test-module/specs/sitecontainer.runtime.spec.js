 ...
 ******************************************************************************/
describe( "Form Runtime with Site Container", () => {

    const pagePath = "content/forms/af/core-components-it/samples/sitecontainer/sitecontainer.html";
    const childBemBlock = 'cmp-adaptiveform-textinput';
    ...
    it(" should get model and view initialized properly and parent child relationship is set, even with site containers in between ", () => {
        ...
        const textInputId = formContainer._model.items[0].items[0].items[0].items[0].id;
        const panelView = formContainer._fields[panelId];
        const textInputView = formContainer._fields[textInputId];
        expect(panelView, "panel view is created").to.not.be.null;
        expect(textInputView, "panel child view is created").to.not.be.null;
        expect(panelView.children.length, "panel has one child").to.equal(1);
        expect(panelView.children[0].id, "panel has reference to child view").to.equal(textInputId);
        expect(textInputView.parentView.id, "text input field has reference to parent panel view").to.equal(panelId);
    })

/** check with repeatability
