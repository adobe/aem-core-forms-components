...
describe("Form Runtime with Turnstile Input", () => {
    ...
    const IS = "adaptiveFormTurnstile"
    const selectors = {
        turnstile : `[data-cmp-is="${IS}"]`
    };
    ...
    it("submission should pass for mandatory captcha", () => {
        if (cy.af.isLatestAddon() && toggle_array.includes(FT_HCAPTCHA)) {
            ...
            cy.visit("mnt/overlay/fd/af/cloudservices/turnstile/properties.html?item=%2Fconf%2Fcore-components-it%2Fsettings%2Fcloudconfigs%2Fturnstile%2Fmanaged").then(x => {
            ...
            expect(formContainer, "formcontainer is initialized").to.not.be.null;
            cy.get(`div.cf-turnstile iframe`).should('be.visible').then($iframe => {
