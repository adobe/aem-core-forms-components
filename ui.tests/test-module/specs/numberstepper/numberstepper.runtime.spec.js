 ...
 ******************************************************************************/
describe("Form with Number Stepper", () => {

    const pagePath = "content/forms/af/core-components-it/samples/numberstepper/basic.html"
    const bemBlock = 'cmp-adaptiveform-numberstepper'
    const IS = "adaptiveFormNumberStepper"
    const selectors = {
        numberstepper : `[data-cmp-is="${IS}"]`
    }
    ...
    it("should show validation error messages based on expression rules", () => {
        ...
        cy.get(`#${numberInput4}`).find("input").clear().type(incorrectInput).blur().then(x => {
            cy.get(`#${numberInput4}`).find(".cmp-adaptiveform-numberstepper__errormessage").should('have.text',"Please enter a valid value.")
        })

        cy.get(`#${numberInput4}`).find("input").clear().type(correctInput).blur().then(x => {
            cy.get(`#${numberInput4}`).find(".cmp-adaptiveform-numberstepper__errormessage").should('have.text',"")
        })
    })
    ...
    it(" should add filled/empty class at container div ", () => {
      ...
      const input = "11.22";
      let model = numberInput7FieldView.getModel();
      cy.get(`#${numberInput7}`).should('have.class', 'cmp-adaptiveform-numberstepper--empty');
      ...
      cy.get(`#${numberInput7}`).find("input").clear().type(input).blur().then(x => {
          expect(Number(model.getState().value)).to.equal(Number(1122));
          cy.get(`#${numberInput7}`).should('have.class', 'cmp-adaptiveform-numberstepper--filled');
      });
    });
})
